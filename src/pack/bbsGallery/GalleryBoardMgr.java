...
public class GalleryBoardMgr {
	...
	public int updateBoard(GalleryBoardBean bean) {
		String sql = null;
		int exeCnt = 0;

		try {
			sql = "update galltblboard set uname=?, subject=?, content=? where num=?";
...
/*  게시판 입력(/bbs/postProc.jsp) 시작  */
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			sql = "select max(num) from galltblboard";
			pStmt = conn.prepareStatement(sql);
			...
			if (multi.getParameter("contentType").equalsIgnoreCase("TEXT")) {
				content = GalleryUtilMgr.replace(content, "<", "&lt;");
			}

			sql = "insert into galltblboard (";
	...
	public Vector<GalleryBoardBean> getBoardList(String keyField, String keyWord, int start, int end) {

		Vector<GalleryBoardBean> vList = new Vector<>();
		String sql = null;
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			...
			if(keyWord.equals("null") || keyWord.equals("")) {
				sql = "select count(*) from galltblboard";
				pStmt = conn.prepareStatement(sql);
			} else {
				sql = "select count(*) from galltblboard ";
	...
	public void upCount(int num) {
		...
		try {
			sql = "update galltblboard set readcnt = readcnt+1 where num=?";
			pStmt = conn.prepareStatement(sql);
			pStmt.setInt(1, num);
			pStmt.executeUpdate();
	...
	public GalleryBoardBean getBoard(int num) {
		...
		GalleryBoardBean bean = new GalleryBoardBean();
		try {
			sql = "select * from galltblboard where num=?";
			pStmt = conn.prepareStatement(sql);
			pStmt.setInt(1, num);
			rS = pStmt.executeQuery();
	...
	*/
	...
	public int deleteBoard(int num) {
		...
		try {
			...
			//////////// 게시글의 파일 삭제 시작 ///////////////
			sql = "select filename from galltblboard where num=?";
			pStmt = conn.prepareStatement(sql);
			pStmt.setInt(1, num);
			rS = pStmt.executeQuery();
			...
			//////////// 게시글 삭제 시작 ///////////////
			sql = "delete from galltblboard where num=?";
			pStmt = conn.prepareStatement(sql);
			pStmt.setInt(1, num);
			exeCnt = pStmt.executeUpdate();
			//////////// 게시글 삭제 끝 ///////////////
	...
	public int replyBoard(GalleryBoardBean bean) {
	...
	public int replyUpBoard(int ref, int pos) {
		...
		try {
			...
			//////////// 게시글의 포지션 증가 시작 ///////////////
			sql = "update galltblboard set pos = pos + 1 ";
